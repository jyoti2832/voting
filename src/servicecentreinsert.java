...
import java.sql.*;
public class servicecentreinsert extends JFrame implements ActionListener{
	JLabel id,name,address,email,phone,regno;
	JTextField id1,name1,address1,email1,phone1,regno1;
	JButton btn,btn1;
	public servicecentreinsert()
	{
		...
		name1.setBounds(550,120,100,30);
		add(name1);
	...
	public static void main(String[] args) {
		...
		servicecentreinsert obj=new servicecentreinsert();
	}

	@Override
	public void actionPerformed(ActionEvent p) {
		// TODO Auto-generated method stub
		if(id1.getText().length()==0 || name1.getText().length()==0 || address1.getText().length()==0 || email1.getText().length()==0)
		{
		...
		else
		{
			try
			{
				...
				int i=Integer.parseInt(id1.getText());
				Class.forName("oracle.jdbc.driver.OracleDriver");
				Connection con=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","system","manager");
				String sql="insert into servicecentre values(?,?,?,?,?,?)";
