...
public class electionscheduleupdate extends JFrame implements ActionListener{
	JLabel a,b,c,d;
	JTextField t2,t3,t4;
	JComboBox t1;
	...
	ResultSet rs;
	public electionscheduleupdate()
	{
		setTitle(" Election Schedule Update");
		setSize(800,800);
		setLayout(null);
		...
		d=new JLabel("Election Date");
		d.setBounds(250,330,200,50);
		d.setFont(new Font("Times New Roman",Font.BOLD,30));
		d.setForeground(Color.BLACK);
		add(d);
		t4=new JTextField();
		t4.setBounds(500,330,120,40);
	...
	public static void main(String[] args) {
		...
		electionscheduleupdate obj=new electionscheduleupdate();
		...
		if(ae.getSource()==btn1)
		{
			try 
			{
				Class.forName("oracle.jdbc.driver.OracleDriver");
				 con=DriverManager.getConnection
					("jdbc:oracle:thin:@localhost:1521:xe","system","manager");
			     String sql="update electionschedule set electiontype=?,cityid=?,electiondate=? where electionid=?";
			     st=con.prepareStatement(sql);
			     ...
			     st.setString(4,t1.getSelectedItem().toString());
