...
public class resultupdate extends JFrame implements ActionListener{
	JLabel a,b,c;
	JTextField t2,t3;
	JComboBox t1;
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if(ae.getSource()==btn)
		{
			try 
			{
				...
				String sql="Select cityid,candidateid from result where wardid=?";
		...
		if(ae.getSource()==btn1)
		{
			try 
			{
				Class.forName("oracle.jdbc.driver.OracleDriver");
				 con=DriverManager.getConnection
					("jdbc:oracle:thin:@localhost:1521:xe","system","manager");
			     String sql="update result set cityid=?,candidateid=? where wardid=?";
			     st=con.prepareStatement(sql);
