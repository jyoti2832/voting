...
public class partymasterupdate extends JFrame implements ActionListener{
	JLabel a,b,c;
	JTextField t2,t3;
	JComboBox t1;
	...
	ResultSet rs;
	public partymasterupdate()
	{
		setTitle(" Party Master Update");
		setSize(800,800);
		setLayout(null);
	...
	public static void main(String[] args) {
		...
		partymasterupdate obj=new partymasterupdate();
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if(ae.getSource()==btn)
		{
			try 
			{
				...
				String sql="Select partyname,symbol from partymaster where partyid=?";
		...
		if(ae.getSource()==btn1)
		{
			try 
			{
				Class.forName("oracle.jdbc.driver.OracleDriver");
				 con=DriverManager.getConnection
					("jdbc:oracle:thin:@localhost:1521:xe","system","manager");
			     String sql="update partymaster set partyname=?,symbol=? where partyid=?";
			     st=con.prepareStatement(sql);
	...
	void filldata()
	{
		try 
		{
			...
			con=DriverManager.getConnection
					("jdbc:oracle:thin:@localhost:1521:xe","System","manager");
			String sql="select partyid from partymaster";
